angular
  .module("dockerpedia.controllers")
  .controller("describeCtrl", describeCtrl);
...
function describeCtrl(scope, location, http) {
  ...
  /* query helpers */
  function valuesQuery(uri, prop, step) {
    //TODO: limit, offset and pagination.
    q = "SELECT DISTINCT ?uri ?label WHERE {\n";
    if (graph) q += "GRAPH ?g {\n";
    q += "  <" + uri + "> <" + prop + "> ?uri .\n";
    q +=
  ...
  /* query helpers */
  function valuesQueryReserve(uri, prop, step) {
    //TODO: limit, offset and pagination.
    q = "SELECT DISTINCT ?uri ?label WHERE {\n";
    if (graph) q += "GRAPH ?g {\n";
    q += "  ?uri  <" + prop + "> <" + uri + ">.\n";
